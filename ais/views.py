...
from utils.response import json_response_error_with_msg, json_response_with_data, response_success

from .aimodels.hkLandFeatures.enter import StartProcess as hongkong_land_features_start_process
from .aimodels.landCoverClassificationUnet.enter import StartProcess as land_cover_start_process
...
def process(request):
    ...
    if (result_uuid is None) or (ai_model_code is None):
        ...
        if ai_model_code == "YOLACT":
            ...
            return json_response_with_data(resData)
        elif ai_model_code == "AirplanesDetect":
            resData = ships_detect_start_process(input_params)
            return json_response_with_data(resData)
        elif ai_model_code == "LandCoverClassificationUnet":
            resData = land_cover_start_process(input_params)
            return json_response_with_data(resData)
